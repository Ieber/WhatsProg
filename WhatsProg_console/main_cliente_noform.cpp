  ...
  while (sock.connected())
  {
    ...
    if (iResult == MYSOCKET_OK)
    {
      ...
      case CMD_NOVA_MSG:
      {
        ...
        if (iResult == MYSOCKET_OK) iResult = sock.read_string(remetente, TIMEOUT_WHATSPROG*1000);
        if (iResult == MYSOCKET_OK) iResult = sock.read_string(texto, TIMEOUT_WHATSPROG*1000);
        if (iResult == MYSOCKET_OK)
        {
    ...
    else // erro na sock.read_int(cmd)
    {
      ...
      // MYSOCKET_ERROR, MYSOCKET_DISCONNECTED ou MYSOCKET_TIMEOUT
      // Se for MYSOCKET_TIMEOUT, aproveita para salvar o arquivo com os dados
...
bool executa_menu_mensagem()
{
  ...
  case 1:
  {
    ...
    for (unsigned i=0; i<it->size(); i++)
    {
      ...
      if (it->getMensagem(i).getRemetente() == DC.getMeuUsuario())
      {
        ...
        cout << " (";
        if (it->getMensagem(i).getStatus() == MSG_ENVIADA) cout << "?";
        if (it->getMensagem(i).getStatus() == MSG_RECEBIDA) cout << "V";
        if (it->getMensagem(i).getStatus() == MSG_ENTREGUE) cout << "VV";
        if (it->getMensagem(i).getStatus() == MSG_LIDA) cout << "VVV";
        cout << ")";
      }
      else
      {
        ...
        if (it->getMensagem(i).getStatus() == MSG_ENTREGUE)
        {
          // Envia msg ao servidor informando que a msg foi lida
          if (envioOK) envioOK = (sock.write_int(CMD_MSG_LIDA1) != MYSOCKET_ERROR);
          if (envioOK) envioOK = (sock.write_int(it->getMensagem(i).getId()) != MYSOCKET_ERROR);
          if (envioOK) envioOK = (sock.write_string(it->getMensagem(i).getRemetente()) != MYSOCKET_ERROR);
          if (envioOK)
          {
            // Muda status na base de dados local
            DC.setStatus(it,i,MSG_LIDA);
          }
        }
      }
      cout << endl;
  ...
  case 2:
  {
    ...
    bool envioOK = (sock.write_int(CMD_NOVA_MSG) != MYSOCKET_ERROR);
    if (envioOK) envioOK = (sock.write_int(M.getId()) != MYSOCKET_ERROR);
    if (envioOK) envioOK = (sock.write_string(M.getDestinatario()) != MYSOCKET_ERROR);
    if (envioOK) envioOK = (sock.write_string(M.getTexto()) != MYSOCKET_ERROR);
    if (envioOK)
    {
      // Move a conversa com a nova msg para o inicio da lista (begin)
      DC.moveConversaToBegin(it);
                   ...
                   const string &senha, bool novoUsuario )
{
  ...
  // Conecta o socket
  if (sock.connect(IP.c_str(), PORTA_WHATSPROG) != MYSOCKET_OK)
  {
    sock.close();
    return false;
  }
  ...
  if (novoUsuario)
  {
    ...
    if (conexaoOK) conexaoOK = (sock.write_int(CMD_NEW_USER) != MYSOCKET_ERROR);
  }
  else
  {
    // Testa se eh possivel se conectar com esses ip, login e senha
    // Envia o comando CMD_LOGIN_USER
    if (conexaoOK) conexaoOK = (sock.write_int(CMD_LOGIN_USER) != MYSOCKET_ERROR);
  }
  if (!conexaoOK) return false;

  // Envia os parametros do comando (login, senha)
  if (conexaoOK) conexaoOK = (sock.write_string(login) != MYSOCKET_ERROR);
  if (conexaoOK) conexaoOK = (sock.write_string(senha) != MYSOCKET_ERROR);
  if (!conexaoOK)
  {
    sock.close();
    return false;
  }
  conexaoOK = (sock.read_int(cmd,1000*TIMEOUT_WHATSPROG) == MYSOCKET_OK);
